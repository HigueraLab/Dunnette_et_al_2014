...
LSEF_lowRes = [573 659 823 891 1360 1577 1890 2065 2870 2948 3316 3888 4081 4179]; 
...
events.nSeries = 2;     % Use two events series
events.x = zeros(length(response.xi),events.nSeries);  % Space for binary
...
for i = 1:nHSCF
    in = find(abs(response.xi - HSCF(i)) ==...
        ...
        % This accomodates event and response series with differing x 
        % values. 
       events.x(in,1) = 1;     % Make this value 1
end
for i = 1:nLSEF_hiRes
    in = find(abs(response.xi - LSEF_hiRes(i)) ==...
        ...
        % series. This accomodates event and response series with differing 
        % x values. 
